/**
 * Serves Week 3 activity content using the Week 1 Activity API contract.
 *
 * Public getActivity responses omit correct answers, mark schemes and tutor notes.
 */

var Week3ActivityDataService = (function () {
  var CONTENT_SCHEMA_VERSION = '1.0';
  var API_VERSION = '1.0';

  var PACK_LOOKUP = {
    WEEK3_PACK_SESSION1_RETRIEVAL: function () {
      return WEEK3_PACK_SESSION1_RETRIEVAL;
    },
    WEEK3_PACK_ATTACKER_TYPES_LEARNING: function () {
      return WEEK3_PACK_ATTACKER_TYPES_LEARNING;
    },
    WEEK3_PACK_ATTACKER_CASE_MATCHING: function () {
      return WEEK3_PACK_ATTACKER_CASE_MATCHING;
    },
    WEEK3_PACK_JUSTIFIED_IDENTIFICATION: function () {
      return WEEK3_PACK_JUSTIFIED_IDENTIFICATION;
    },
    WEEK3_PACK_SESSION2_RETRIEVAL: function () {
      return WEEK3_PACK_SESSION2_RETRIEVAL;
    },
    WEEK3_PACK_OCR_PRACTICE: function () {
      return WEEK3_PACK_OCR_PRACTICE;
    },
    WEEK3_PACK_PEER_MARKING: function () {
      return WEEK3_PACK_PEER_MARKING;
    }
  };
    ...
    if (action === 'getActivity' || action === 'activity') {
      ...
      if (!pack) {
        return errorEnvelope_(
          ...
          action,
          'UNKNOWN_ACTIVITY',
          'Activity ID is not recognised for Week 3.'
        );
      }
  ...
  function healthEnvelope_(requestId) {
    return ResponseFactory.apiEnvelope({
      ...
      data: {
        build: 'UNIT3-WEEK3-ACTIVITY-API-V1.0',
  ...
  function buildManifestPayload_() {
    var activities = getWeek3ManifestIds_().map(function (id) {
      var entry = getWeek3ManifestEntry_(id);
      return {
  ...
  function markSection_(requestId, body) {
    ...
    if (!pack) {
      return errorEnvelope_(requestId, 'markSection', 'UNKNOWN_ACTIVITY', 'Activity ID is not recognised for Week 3.');
    }
  ...
  function submitAttempt_(requestId, body) {
    if (!areWeek3SubmissionsOpen_()) {
      return errorEnvelope_(
        requestId,
        'submitAttempt',
        'SUBMISSIONS_CLOSED',
        'Week 3 submissions are currently closed.'
      );
    }
  ...
  function bootstrapSetup_(requestId, params) {
    ...
    if (props.getProperty('WEEK3_BOOTSTRAP_DONE') === 'true') {
      return errorEnvelope_(
        requestId,
        'bootstrapSetup',
        'BOOTSTRAP_DONE',
        'Week 3 bootstrap has already been completed.'
      );
    }
    if (String(params.confirm || '') !== 'Unit3-Week3-Bootstrap-Once') {
      return errorEnvelope_(
        requestId,
        'bootstrapSetup',
        'BOOTSTRAP_FORBIDDEN',
    ...
    try {
      var summary = runWeek3DeploymentBootstrap();
      props.setProperty('WEEK3_BOOTSTRAP_DONE', 'true');
  ...
  function parseJsonBody_(e) {
    ...
    try {
      ...
      Logger.log('Week3ActivityDataService parse error: ' + err);
