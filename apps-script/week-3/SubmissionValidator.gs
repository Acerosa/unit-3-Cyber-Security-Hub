/**
 * Validates normalised Week 3 submissions against the activity registry.
 */
  ...
  function validate(submission) {
    ...
    if (submission.weekNumber !== CONFIG.weekNumber) {
      errors.push({
        ...
        message: 'Week number must be 2 for this API.'
      });
    }
    ...
    if (!submission.activityId) {
      ...
      var activity = getWeek3Activity_(submission.activityId);
      if (!activity) {
        errors.push({
          field: 'activityId',
          code: 'UNKNOWN_ACTIVITY',
          message: 'Activity ID is not recognised for Week 3.'
        });
      } else {
    ...
    if (submission.score === null || submission.score === undefined || typeof submission.score !== 'number' || !isFinite(submission.score)) {
      ...
      var registered = getWeek3Activity_(submission.activityId);
      if (registered) {
