/**
 * Creates and repairs Week 6 worksheet tabs in the shared Unit 3 spreadsheet.
 *
 * Existing Week 1 tabs and learner results are preserved.
 */

/**
 * Sets up Week 6 tabs, headers and formatting.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function setupWeek6Workbook() {
  var messages = [];
  ...
  formatWeek6ResultsSheet_(spreadsheet.getSheetByName(CONFIG.weekResultsSheetName));
  formatErrorsSheet_(spreadsheet.getSheetByName(CONFIG.errorsSheetName));

  ensureWeek6ActivityCatalogueSheet_({ messages: messages });
  messages.push('Week 6 Activity Catalogue is ready for seedAllWeek6ActivityData().');

  messages.push('Week 6 workbook setup complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}

/**
 * Repairs missing Week 6 tabs or headers without clearing learner data.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function repairWeek6Workbook() {
  var messages = [];
  var spreadsheet = getWorkbook_();
  ...
  messages.push('Week 6 workbook repair complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}
...
function formatWeek6ResultsSheet_(sheet) {
  if (!sheet) return;
  applyHeaderStyle_(sheet, WEEK_6_RESULTS_HEADERS.length);
  sheet.setColumnWidths(1, WEEK_6_RESULTS_HEADERS.length, 120);
  sheet.setColumnWidth(1, 160);
  sheet.setColumnWidth(2, 160);
