/**
 * Validates normalised Week 2 submissions against the activity registry.
 */
...
var SubmissionValidator = (function () {
  ...
  function validate(submission) {
    ...
    if (!submission.activityId) {
      ...
      var activity = getWeek2Activity_(submission.activityId);
      if (!activity) {
        errors.push({
          field: 'activityId',
          code: 'UNKNOWN_ACTIVITY',
          message: 'Activity ID is not recognised for Week 2.'
        });
      } else {
    ...
    if (submission.score === null || submission.score === undefined || typeof submission.score !== 'number' || !isFinite(submission.score)) {
      ...
      var registered = getWeek2Activity_(submission.activityId);
      if (registered) {
