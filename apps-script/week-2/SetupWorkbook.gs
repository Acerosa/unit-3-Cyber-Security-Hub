/**
 * Creates and repairs Week 2 worksheet tabs in the shared Unit 3 spreadsheet.
 *
 * Existing Week 1 tabs and learner results are preserved.
 */

/**
 * Sets up Week 2 tabs, headers and formatting.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function setupWeek2Workbook() {
  var messages = [];
...
/**
 * Repairs missing Week 2 tabs or headers without clearing learner data.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function repairWeek2Workbook() {
  var messages = [];
  var spreadsheet = getWorkbook_();
  ...
  messages.push('Week 2 workbook repair complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}
...
function formatWeek2ResultsSheet_(sheet) {
  if (!sheet) return;
  applyHeaderStyle_(sheet, WEEK_2_RESULTS_HEADERS.length);
  sheet.setColumnWidths(1, WEEK_2_RESULTS_HEADERS.length, 120);
  sheet.setColumnWidth(1, 160);
  sheet.setColumnWidth(2, 160);
