/**
 * Runnable self-tests for the Week 2 Apps Script API.
 *
 * Ordinary self-tests do not write fake learner submissions.
 */

/**
 * Runs every Week 2 self-test and throws when any assertion fails.
 */
function runAllWeek2SelfTests() {
  var results = [];
  results.push(runNamedTest_('Activity registry', runWeek2ActivityRegistrySelfTest));
  results.push(runNamedTest_('Totals', runWeek2TotalsSelfTest));
  results.push(runNamedTest_('Versions', runWeek2VersionsSelfTest));
  results.push(runNamedTest_('Validation', runWeek2ValidationSelfTest));
  results.push(runNamedTest_('Duplicate keys', runWeek2DuplicateKeySelfTest));
  results.push(runNamedTest_('Spreadsheet connection', runWeek2SpreadsheetConnectionTest));
  ...
  Logger.log('Week 2 self-test summary\n' + summary);

  if (failed.length > 0) {
    throw new Error(
      'Week 2 self-tests failed (' + failed.length + '):\n' + summary
    );
  }

  Logger.log('All Week 2 self-tests passed.');
  return summary;
}

function runWeek2ActivityRegistrySelfTest() {
  var ids = getWeek2ActivityIds_();
  assert_(ids.length === 11, 'Expected 11 Week 2 activities, found ' + ids.length);
  ...
  ids.forEach(function (id) {
    ...
    var activity = getWeek2Activity_(id);
    assert_(!!activity, 'Missing activity record for ' + id);
    assert_(activity.week === 2, id + ' must belong to week 2');
  ...
  assert_(!getWeek2Activity_('not-a-real-activity'), 'Unknown activities must not resolve');
  assert_(!isWeek2ActivityEnabled_('not-a-real-activity'), 'Unknown activities must not be enabled');
}

function runWeek2TotalsSelfTest() {
  var expected = {
    'week2-session1-retrieval': 10,
    'week2-threat-vulnerability-learning': 6,
    'week2-malware-symptoms': 10,
    'week2-threat-vulnerability-sort': 12,
    'week2-vulnerabilities101-reflection': 2,
    'week2-session2-retrieval': 10,
    'week2-northbank-vulnerability-analysis': 5,
    'week2-six-mark-response-guide': 3,
    'week2-ocr-question-practice': 20,
    'week2-peer-marking-answer-improvement': 6,
    'week2-northbank-vulnerability-register': 5
  };
...
function runWeek2VersionsSelfTest() {
  getWeek2ActivityIds_().forEach(function (id) {
    var activity = getWeek2Activity_(id);
    assert_(activity.version === '1.0', id + ' version must be 1.0');
  });
}

function runWeek2ValidationSelfTest() {
  ...
  var unknown = SubmissionValidator.validate(
    buildValidSubmission_({ activityId: 'week2-does-not-exist' })
  );
  ...
  assert_(!wrongWeek.valid, 'Week 1 submissions must be rejected by the Week 2 API');
...
function runWeek2DuplicateKeySelfTest() {
...
function runWeek2SpreadsheetConnectionTest() {
  assert_(
  ...
  required.forEach(function (sheetName) {
    ...
    assert_(
      ...
      'Required tab missing: ' +
        sheetName +
        '. Run setupWeek2Workbook() before relying on sheet writes.'
    );
  });
}
 ...
 * Not called by runAllWeek2SelfTests().
 */
function runWeek2WriteCleanupIntegrationTest() {
  var lock = LockService.getScriptLock();
  lock.waitLock(30000);

  var submission = buildValidSubmission_({
    learnerName: 'TEST DATA — Week 2 API Self-Check',
  ...
  try {
    ...
    cleanupWeek2TestSubmission_(submission.submissionKey);
    assert_(
 ...
 * and Week 2 Results. Intended for TEST DATA cleanup only.
 *
 * @param {string} submissionKey
 */
function cleanupWeek2TestSubmission_(submissionKey) {
  deleteRowsByKey_(CONFIG.submissionsSheetName, ALL_SUBMISSIONS_HEADERS, submissionKey);
  deleteRowsByKey_(CONFIG.weekResultsSheetName, WEEK_2_RESULTS_HEADERS, submissionKey);
}
