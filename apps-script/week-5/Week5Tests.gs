/**
 * Runnable self-tests for the Week 5 Apps Script API.
 *
 * Ordinary self-tests do not write fake learner submissions.
 */

/**
 * Runs every Week 5 self-test and throws when any assertion fails.
 */
function runAllWeek5SelfTests() {
  var results = [];
  results.push(runNamedTest_('Activity registry', runWeek5ActivityRegistrySelfTest));
  results.push(runNamedTest_('Totals', runWeek5TotalsSelfTest));
  results.push(runNamedTest_('Versions', runWeek5VersionsSelfTest));
  results.push(runNamedTest_('Validation', runWeek5ValidationSelfTest));
  results.push(runNamedTest_('Duplicate keys', runWeek5DuplicateKeySelfTest));
  results.push(runNamedTest_('Spreadsheet connection', runWeek5SpreadsheetConnectionTest));
  ...
  Logger.log('Week 5 self-test summary\n' + summary);

  if (failed.length > 0) {
    throw new Error('Week 5 self-tests failed (' + failed.length + '):\n' + summary);
  }

  Logger.log('All Week 5 self-tests passed.');
  return summary;
}

/**
 * Preferred alias used in Week 5 documentation.
 *
 * @return {string}
 */
function runWeek5SelfTest() {
  return runAllWeek5SelfTests();
}

function runWeek5ActivityRegistrySelfTest() {
  var ids = getWeek5ActivityIds_();
  assert_(ids.length === 10, 'Expected 10 Week 5 activities, found ' + ids.length);
  ...
  ids.forEach(function (id) {
    ...
    var activity = getWeek5Activity_(id);
    assert_(!!activity, 'Missing activity record for ' + id);
    assert_(activity.week === 5, id + ' must belong to week 5');
  ...
  assert_(!getWeek5Activity_('not-a-real-activity'), 'Unknown activities must not resolve');
  assert_(!isWeek5ActivityEnabled_('not-a-real-activity'), 'Unknown activities must not be enabled');
}

function runWeek5TotalsSelfTest() {
  var expected = {
    'week5-session1-retrieval': 8,
    'week5-impacts-learning': 9,
    'week5-impact-classification': 8,
    'week5-ransomware-companion': 4,
    'week5-exercise-debrief': 4,
    'week5-session2-retrieval': 12,
    'week5-stakeholder-grid': 10,
    'week5-impact-analysis': 6,
    'week5-ocr-question-practice': 20,
    'week5-answer-improvement': 6
  };
...
function runWeek5VersionsSelfTest() {
  getWeek5ActivityIds_().forEach(function (id) {
    var activity = getWeek5Activity_(id);
    assert_(activity.version === '1.0', id + ' version must be 1.0');
  });
}

function runWeek5ValidationSelfTest() {
  var valid = SubmissionValidator.validate(buildValidWeek5Submission_());
  ...
  var unknown = SubmissionValidator.validate(
    buildValidWeek5Submission_({ activityId: 'week5-does-not-exist' })
  );
  ...
  var badTotal = SubmissionValidator.validate(buildValidWeek5Submission_({ total: 99 }));
  ...
  var badVersion = SubmissionValidator.validate(
    buildValidWeek5Submission_({ activityVersion: '9.9' })
  );
  ...
  var aboveMax = SubmissionValidator.validate(buildValidWeek5Submission_({ score: 13 }));
  ...
  var negative = SubmissionValidator.validate(buildValidWeek5Submission_({ score: -1 }));
  ...
  var wrongWeek = SubmissionValidator.validate(buildValidWeek5Submission_({ weekNumber: 1 }));
  assert_(!wrongWeek.valid, 'Week 1 submissions must be rejected by the Week 5 API');
...
function runWeek5DuplicateKeySelfTest() {
  var base = buildValidWeek5Submission_();
  ...
  var laterAttempt = buildValidWeek5Submission_({ attemptNumber: 2 });
  ...
  var byId = DuplicateChecker.buildSubmissionKey(
    buildValidWeek5Submission_({ learnerId: 'STU999', learnerName: 'Different Name' })
  );
  var byNameOnly = DuplicateChecker.buildSubmissionKey(
    buildValidWeek5Submission_({ learnerId: '', learnerName: 'Student Name' })
  );
  assert_(byId !== byNameOnly, 'Learner ID and learner-name keys should differ when IDs differ');
}

function runWeek5SpreadsheetConnectionTest() {
  assert_(
  ...
  required.forEach(function (sheetName) {
    ...
    assert_(
      ...
      'Required tab missing: ' +
        sheetName +
        '. Run setupWeek5Workbook() before relying on sheet writes.'
    );
  });
}
 ...
 * Not called by runAllWeek5SelfTests().
 */
function runWeek5WriteCleanupIntegrationTest() {
  var lock = LockService.getScriptLock();
  lock.waitLock(30000);

  var submission = buildValidWeek5Submission_({
    learnerName: 'TEST DATA — Week 5 API Self-Check',
    learnerId: 'TEST-WEEK5-API',
  ...
  try {
    ...
    cleanupWeek5TestSubmission_(submission.submissionKey);
    assert_(
 ...
 * and Week 5 Results. Intended for TEST DATA cleanup only.
 *
 * @param {string} submissionKey
 */
function cleanupWeek5TestSubmission_(submissionKey) {
  deleteRowsByKey_(CONFIG.submissionsSheetName, ALL_SUBMISSIONS_HEADERS, submissionKey);
  deleteRowsByKey_(CONFIG.weekResultsSheetName, WEEK_5_RESULTS_HEADERS, submissionKey);
}
...
function buildValidWeek5Submission_(overrides) {
  var base = {
    ...
    total: 12,
    attemptNumber: 1,
    completedAt: '2026-08-06T18:30:00.000Z',
