/**
 * Creates and repairs Week 5 worksheet tabs in the shared Unit 3 spreadsheet.
 *
 * Existing Week 1 tabs and learner results are preserved.
 */

/**
 * Sets up Week 5 tabs, headers and formatting.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function setupWeek5Workbook() {
  var messages = [];
  ...
  formatWeek5ResultsSheet_(spreadsheet.getSheetByName(CONFIG.weekResultsSheetName));
  formatErrorsSheet_(spreadsheet.getSheetByName(CONFIG.errorsSheetName));

  ensureWeek5ActivityCatalogueSheet_({ messages: messages });
  messages.push('Week 5 Activity Catalogue is ready for seedAllWeek5ActivityData().');

  messages.push('Week 5 workbook setup complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}

/**
 * Repairs missing Week 5 tabs or headers without clearing learner data.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function repairWeek5Workbook() {
  var messages = [];
  var spreadsheet = getWorkbook_();
  ...
  messages.push('Week 5 workbook repair complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}
...
function formatWeek5ResultsSheet_(sheet) {
  if (!sheet) return;
  applyHeaderStyle_(sheet, WEEK_5_RESULTS_HEADERS.length);
  sheet.setColumnWidths(1, WEEK_5_RESULTS_HEADERS.length, 120);
  sheet.setColumnWidth(1, 160);
  sheet.setColumnWidth(2, 160);
