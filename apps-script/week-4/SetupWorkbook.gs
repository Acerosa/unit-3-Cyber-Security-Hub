/**
 * Creates and repairs Week 4 worksheet tabs in the shared Unit 3 spreadsheet.
 *
 * Existing Week 1 tabs and learner results are preserved.
 */

/**
 * Sets up Week 4 tabs, headers and formatting.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function setupWeek4Workbook() {
  var messages = [];
  ...
  formatWeek4ResultsSheet_(spreadsheet.getSheetByName(CONFIG.weekResultsSheetName));
  formatErrorsSheet_(spreadsheet.getSheetByName(CONFIG.errorsSheetName));

  ensureWeek4ActivityCatalogueSheet_({ messages: messages });
  messages.push('Week 4 Activity Catalogue is ready for seedAllWeek4ActivityData().');

  messages.push('Week 4 workbook setup complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}

/**
 * Repairs missing Week 4 tabs or headers without clearing learner data.
 *
 * @return {{ok: boolean, messages: string[]}}
 */
function repairWeek4Workbook() {
  var messages = [];
  var spreadsheet = getWorkbook_();
  ...
  messages.push('Week 4 workbook repair complete.');
  Logger.log(messages.join('\n'));
  return { ok: true, messages: messages };
}
...
function formatWeek4ResultsSheet_(sheet) {
  if (!sheet) return;
  applyHeaderStyle_(sheet, WEEK_4_RESULTS_HEADERS.length);
  sheet.setColumnWidths(1, WEEK_4_RESULTS_HEADERS.length, 120);
  sheet.setColumnWidth(1, 160);
  sheet.setColumnWidth(2, 160);
