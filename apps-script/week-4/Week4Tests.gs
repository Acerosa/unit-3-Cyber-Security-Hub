/**
 * Runnable self-tests for the Week 4 Apps Script API.
 *
 * Ordinary self-tests do not write fake learner submissions.
 */

/**
 * Runs every Week 4 self-test and throws when any assertion fails.
 */
function runAllWeek4SelfTests() {
  var results = [];
  results.push(runNamedTest_('Activity registry', runWeek4ActivityRegistrySelfTest));
  results.push(runNamedTest_('Totals', runWeek4TotalsSelfTest));
  results.push(runNamedTest_('Versions', runWeek4VersionsSelfTest));
  results.push(runNamedTest_('Validation', runWeek4ValidationSelfTest));
  results.push(runNamedTest_('Duplicate keys', runWeek4DuplicateKeySelfTest));
  results.push(runNamedTest_('Spreadsheet connection', runWeek4SpreadsheetConnectionTest));
  ...
  Logger.log('Week 4 self-test summary\n' + summary);

  if (failed.length > 0) {
    throw new Error(
      'Week 4 self-tests failed (' + failed.length + '):\n' + summary
    );
  }

  Logger.log('All Week 4 self-tests passed.');
  return summary;
}

/**
 * Preferred alias used in Week 4 documentation.
 *
 * @return {string}
 */
function runWeek4SelfTest() {
  return runAllWeek4SelfTests();
}

function runWeek4ActivityRegistrySelfTest() {
  var ids = getWeek4ActivityIds_();
  assert_(ids.length === 10, 'Expected 10 Week 4 activities, found ' + ids.length);
  ...
  ids.forEach(function (id) {
    ...
    var activity = getWeek4Activity_(id);
    assert_(!!activity, 'Missing activity record for ' + id);
    assert_(activity.week === 4, id + ' must belong to week 4');
  ...
  assert_(!getWeek4Activity_('not-a-real-activity'), 'Unknown activities must not resolve');
  assert_(!isWeek4ActivityEnabled_('not-a-real-activity'), 'Unknown activities must not be enabled');
}

function runWeek4TotalsSelfTest() {
  var expected = {
    'week4-session1-retrieval': 10,
    'week4-motivations-learning': 8,
    'week4-targets-methods': 8,
    'week4-northbank-exposure': 3,
    'week4-session2-retrieval': 12,
    'week4-mtm-mapping': 8,
    'week4-analyse-practice': 6,
    'week4-ocr-question-practice': 20,
    'week4-answer-improvement': 6,
    'week4-ethical-review': 2
  };
...
function runWeek4VersionsSelfTest() {
  getWeek4ActivityIds_().forEach(function (id) {
    var activity = getWeek4Activity_(id);
    assert_(activity.version === '1.0', id + ' version must be 1.0');
  });
}

function runWeek4ValidationSelfTest() {
  ...
  var unknown = SubmissionValidator.validate(
    buildValidSubmission_({ activityId: 'week4-does-not-exist' })
  );
  ...
  assert_(!wrongWeek.valid, 'Week 1 submissions must be rejected by the Week 4 API');
...
function runWeek4DuplicateKeySelfTest() {
...
function runWeek4SpreadsheetConnectionTest() {
  assert_(
  ...
  required.forEach(function (sheetName) {
    ...
    assert_(
      ...
      'Required tab missing: ' +
        sheetName +
        '. Run setupWeek4Workbook() before relying on sheet writes.'
    );
  });
}
 ...
 * Not called by runAllWeek4SelfTests().
 */
function runWeek4WriteCleanupIntegrationTest() {
  var lock = LockService.getScriptLock();
  lock.waitLock(30000);

  var submission = buildValidSubmission_({
    learnerName: 'TEST DATA — Week 4 API Self-Check',
 ...
 * and Week 4 Results. Intended for TEST DATA cleanup only.
 *
 * @param {string} submissionKey
 */
function cleanupWeek4TestSubmission_(submissionKey) {
  deleteRowsByKey_(CONFIG.submissionsSheetName, ALL_SUBMISSIONS_HEADERS, submissionKey);
  deleteRowsByKey_(CONFIG.weekResultsSheetName, WEEK_4_RESULTS_HEADERS, submissionKey);
}
