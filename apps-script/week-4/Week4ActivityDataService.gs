/**
 * Serves Week 4 activity content using the Week 1 Activity API contract.
 *
 * Public getActivity responses omit correct answers, mark schemes and tutor notes.
 */

var Week4ActivityDataService = (function () {
  var CONTENT_SCHEMA_VERSION = '1.0';
  var API_VERSION = '1.0';

  var PACK_LOOKUP = {
    WEEK4_PACK_SESSION1_RETRIEVAL: function () {
      return WEEK4_PACK_SESSION1_RETRIEVAL;
    },
    WEEK4_PACK_MOTIVATIONS_LEARNING: function () {
      return WEEK4_PACK_MOTIVATIONS_LEARNING;
    },
    WEEK4_PACK_TARGETS_METHODS: function () {
      return WEEK4_PACK_TARGETS_METHODS;
    },
    WEEK4_PACK_NORTHBANK_EXPOSURE: function () {
      return WEEK4_PACK_NORTHBANK_EXPOSURE;
    },
    WEEK4_PACK_SESSION2_RETRIEVAL: function () {
      return WEEK4_PACK_SESSION2_RETRIEVAL;
    },
    WEEK4_PACK_MTM_MAPPING: function () {
      return WEEK4_PACK_MTM_MAPPING;
    },
    WEEK4_PACK_ANALYSE_PRACTICE: function () {
      return WEEK4_PACK_ANALYSE_PRACTICE;
    },
    WEEK4_PACK_OCR_PRACTICE: function () {
      return WEEK4_PACK_OCR_PRACTICE;
    },
    WEEK4_PACK_ANSWER_IMPROVEMENT: function () {
      return WEEK4_PACK_ANSWER_IMPROVEMENT;
    },
    WEEK4_PACK_ETHICAL_REVIEW: function () {
      return WEEK4_PACK_ETHICAL_REVIEW;
    }
  };
    ...
    if (action === 'getActivity' || action === 'activity') {
      ...
      if (!pack) {
        return errorEnvelope_(
          ...
          action,
          'UNKNOWN_ACTIVITY',
          'Activity ID is not recognised for Week 4.'
        );
      }
  ...
  function healthEnvelope_(requestId) {
    return ResponseFactory.apiEnvelope({
      ...
      data: {
        build: 'UNIT3-WEEK4-ACTIVITY-API-V1.0',
  ...
  function buildManifestPayload_() {
    var activities = getWeek4ManifestIds_().map(function (id) {
      var entry = getWeek4ManifestEntry_(id);
      return {
  ...
  function markSection_(requestId, body) {
    ...
    if (!pack) {
      return errorEnvelope_(requestId, 'markSection', 'UNKNOWN_ACTIVITY', 'Activity ID is not recognised for Week 4.');
    }
  ...
  function markQuestion_(question, value, assessment) {
    ...
    if (assessment.requiresEvidence === true && isCorrect) {
      return {
        ...
        feedback:
          'A defensible motivation was selected. Supporting evidence is still required for full credit.',
        explanation: assessment.explanation || ''
      };
    }
  ...
  function submitAttempt_(requestId, body) {
    if (!areWeek4SubmissionsOpen_()) {
      return errorEnvelope_(
        requestId,
        'submitAttempt',
        'SUBMISSIONS_CLOSED',
        'Week 4 submissions are currently closed.'
      );
    }
  ...
  function bootstrapSetup_(requestId, params) {
    ...
    if (props.getProperty('WEEK4_BOOTSTRAP_DONE') === 'true') {
      return errorEnvelope_(
        requestId,
        'bootstrapSetup',
        'BOOTSTRAP_DONE',
        'Week 4 bootstrap has already been completed.'
      );
    }
    if (String(params.confirm || '') !== 'Unit3-Week4-Bootstrap-Once') {
      return errorEnvelope_(
        requestId,
        'bootstrapSetup',
        'BOOTSTRAP_FORBIDDEN',
    ...
    try {
      var summary = runWeek4DeploymentBootstrap();
      props.setProperty('WEEK4_BOOTSTRAP_DONE', 'true');
  ...
  function parseJsonBody_(e) {
    ...
    try {
      ...
      Logger.log('Week4ActivityDataService parse error: ' + err);
