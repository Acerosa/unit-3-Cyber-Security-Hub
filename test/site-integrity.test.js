...
test("GitHub Pages consumes reviewed Core 0.2.25, UI 0.1.14 and Content 0.1.4", function () {
  const workflow = read(".github/workflows/pages.yml");
  assert.match(workflow, /Check out reviewed UI/);
  assert.match(workflow, /Acerosa\/Acerosa-learning-platform-ui/);
  assert.match(workflow, /ref:\s*v0\.1\.14/);
  assert.match(workflow, /ref:\s*v0\.2\.25/);
...
test("week pages keep docked catalogue progress chrome", function () {
  ...
  assert.match(read("src/pages/PageHost.tsx"), /Unit3BackendProgress\?\.reconcile/);
