...
function load({ progress, signedIn = true, storage } = {}) {
  const window = {
    ...
    Unit3ActivityKeyMap: {
      ...
      normaliseActivityVersion: (version) => version || "1.2.0"
    },
    LearningPlatform: {
      platform: {
        auth: {
...
test("Retry replaces the stored question response without appending", () => {
    ...
    progress: {
      createStore() {
        return {
          ...
          save(payload, options) { saves.push({ payload, options }); }
        };
      }
    }
  });
  const state = api.load("U3-W01-CIA");
  state.activityVersion = "1.2.0";
  api.setResponse(state, "Q1", "A");
  api.setChecked(state, "Q1", true);
  api.setResponse(state, "Q1", "C");
  api.setChecked(state, "Q1", true);
  assert.equal(api.load("U3-W01-CIA").responses.Q1, "C");
  assert.equal(api.load("U3-W01-CIA").checked.Q1, true);
  assert.equal(Object.keys(api.load("U3-W01-CIA").responses).length, 1);
});
