...
### Academic integrity (site-wide controls)

Shared helper: `js/academic-integrity.js` (loaded via React `sharedAdapters` and from `js/navigation.js` on classic hub pages).
...
- **Blocks paste and drop** on host learning textareas/inputs (aligned with catalogue `ShortResponse` / `Reflection`).
- **Excluded:** catalogue library fields (`[data-lp-block="short-response|reflection"]`), learner details forms, partner fields, and other opted-out controls.
- Tutors should still review responses for typed walkthrough text.
