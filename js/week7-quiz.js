/**
 * Reusable one-question-at-a-time quiz engine for Week 7 activities.
 */
...
(function (global) {
  ...
  function createQuiz(config) {
    ...
    var host = document.getElementById(config.hostId || 'w7-quiz-host');
    ...
    function render() {
      ...
      panel.setAttribute('aria-labelledby', 'w7-q-heading');
      ...
      (q.options || []).forEach(function (option, optionIndex) {
        var id = 'w7-option-' + index + '-' + optionIndex;
        var label = document.createElement('label');
        label.className = 'w7-option';
        label.setAttribute('for', id);
        var input = document.createElement('input');
        input.type = 'radio';
        input.name = 'w7-answer';
        input.id = id;
      ...
      checkBtn.addEventListener('click', function () {
        if (locked) return;
        var selected = host.querySelector('input[name="w7-answer"]:checked');
        if (!selected) {
        ...
        host.querySelectorAll('input[name="w7-answer"]').forEach(function (input) {
          input.disabled = true;
        });
        feedback.textContent = '';
    ...
    function renderSummary() {
      var panel = document.createElement('section');
      panel.className = 'activity-panel';
      panel.setAttribute('aria-labelledby', 'w7-summary-heading');

      var heading = document.createElement('h2');
      heading.id = 'w7-summary-heading';
      ...
      review.className = 'w7-review-list';
