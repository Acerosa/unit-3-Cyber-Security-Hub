/**
 * Reusable one-question-at-a-time quiz engine for Week 2 activities.
 */
...
(function (global) {
  ...
  function createQuiz(config) {
    ...
    var host = document.getElementById(config.hostId || 'w2-quiz-host');
    ...
    function render() {
      ...
      panel.setAttribute('aria-labelledby', 'w2-q-heading');
      ...
      (q.options || []).forEach(function (option, optionIndex) {
        var id = 'w2-option-' + index + '-' + optionIndex;
        var label = document.createElement('label');
        label.className = 'w2-option';
        label.setAttribute('for', id);
        var input = document.createElement('input');
        input.type = 'radio';
        input.name = 'w2-answer';
        input.id = id;
      ...
      checkBtn.addEventListener('click', function () {
        if (locked) return;
        var selected = host.querySelector('input[name="w2-answer"]:checked');
        if (!selected) {
        ...
        host.querySelectorAll('input[name="w2-answer"]').forEach(function (input) {
          input.disabled = true;
        });
        feedback.textContent = '';
    ...
    function renderSummary() {
      var panel = document.createElement('section');
      panel.className = 'activity-panel';
      panel.setAttribute('aria-labelledby', 'w2-summary-heading');

      var heading = document.createElement('h2');
      heading.id = 'w2-summary-heading';
      ...
      review.className = 'w2-review-list';
      ...
      if (incorrect.length === 0) {
        ...
        answers.forEach(function (item, i) {
          ...
          block.className = 'w2-review-item';
