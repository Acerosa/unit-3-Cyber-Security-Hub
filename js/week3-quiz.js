/**
 * Reusable one-question-at-a-time quiz engine for Week 3 activities.
 */
...
(function (global) {
  ...
  function createQuiz(config) {
    ...
    function render() {
      ...
      panel.setAttribute('aria-labelledby', 'w3-q-heading');
      ...
      (q.options || []).forEach(function (option, optionIndex) {
        var id = 'w3-option-' + index + '-' + optionIndex;
        var label = document.createElement('label');
        label.className = 'w3-option';
        label.setAttribute('for', id);
        var input = document.createElement('input');
        input.type = 'radio';
        input.name = 'w3-answer';
        input.id = id;
      ...
      checkBtn.addEventListener('click', function () {
        if (locked) return;
        var selected = host.querySelector('input[name="w3-answer"]:checked');
        if (!selected) {
        ...
        host.querySelectorAll('input[name="w3-answer"]').forEach(function (input) {
          input.disabled = true;
        });
        feedback.textContent = '';
    ...
    function renderSummary() {
      var panel = document.createElement('section');
      panel.className = 'activity-panel';
      panel.setAttribute('aria-labelledby', 'w3-summary-heading');

      var heading = document.createElement('h2');
      heading.id = 'w3-summary-heading';
      ...
      review.className = 'w3-review-list';
