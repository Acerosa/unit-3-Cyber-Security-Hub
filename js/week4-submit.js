/**
 * Shared Week 4 submission helper.
 * Posts JSON results to the Week 4 Apps Script /exec URL.
 */
...
(function (global) {
  ...
  function getWeek4ApiUrl() {
    var modules = getModules();
    if (modules.config && typeof modules.config.getWeek4ApiBaseUrl === 'function') {
      return modules.config.getWeek4ApiBaseUrl();
    }
  ...
  function renderSubmitPanel(options) {
    var modules = getModules();
    var hostId = options.hostId || 'w4-submit-host';
    ...
    courseHost.id = 'w4-course-details';
    ...
    summary.id = 'w4-submission-summary';
  ...
  function buildWeek4Payload(activity, learner, options, attemptId, attemptNumber) {
    return {
  ...
  function postWeek4Submission(payload) {
    var url = getWeek4ApiUrl();
    if (!url || !/\/exec\/?$/.test(url)) {
      return Promise.reject(new Error('Week 4 API URL is not configured.'));
    }
    ...
    return fetch(url, {
      ...
      return response.text().then(function (text) {
        ...
        try {
          ...
          throw new Error('The Week 4 API returned an unexpected response.');
        }
  ...
  function submitResult(options) {
    var modules = getModules();
    if (submitting) {
      setMessage('w4-submit-status', 'Submission already in progress. Please wait.', 'warning');
      return;
    }
    ...
    if (!activity) {
      ...
      console.error('[Week4Submit] Missing registry entry for', options.activityId);
      return;
    }
    ...
    modules.learner.showValidationSummary('w4-submit-errors', validation);
    if (!validation.valid) {
      setMessage('w4-submit-status', 'Check your details and try again.', 'warning');
      return;
    }
    ...
    if (!Number.isInteger(score) || score < 0 || score > total) {
      ...
      console.error('[Week4Submit] Invalid score', score, total);
      return;
    }
    ...
    if (total !== activity.maximumScore) {
      ...
      console.error('[Week4Submit] Total mismatch', total, activity.maximumScore);
      return;
    }

    submitting = true;
    var btn = document.getElementById('w4-btn-submit');
    if (btn) {
      btn.disabled = true;
      btn.textContent = 'Submitting…';
    }
    setMessage('w4-submit-status', 'Submitting your formative result…', 'info');

    var payload = buildWeek4Payload(
      activity,
      validation.learner,
      {
        score: score,
      ...
      .then(function (result) {
        ...
        if (envelope.ok === true && (envelope.recorded === true || envelope.duplicate === true)) {
          ...
          if (modules.learner.renderSubmissionSummary) {
            modules.learner.renderSubmissionSummary('w4-submission-summary', {
          ...
          if (typeof options.onSubmitted === 'function') {
            try {
              ...
              console.error('[Week4Submit] onSubmitted callback failed', callbackErr);
            }
          }
          return;
        }
        ...
        setMessage('w4-submit-status', 'Your result was not saved. ' + detail, 'error');
      })
      .catch(function (err) {
        console.error('[Week4Submit] Submission failed', err);
        if (btn) {
          btn.disabled = false;
          btn.textContent = 'Submit formative result';
        }
