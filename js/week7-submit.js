/**
 * Shared Week 7 submission helper.
 * Posts JSON results to the Week 7 Apps Script /exec URL.
 */
...
(function (global) {
  ...
  function getWeek7ApiUrl() {
    var modules = getModules();
    if (modules.config && typeof modules.config.getWeek7ApiBaseUrl === 'function') {
      return modules.config.getWeek7ApiBaseUrl();
    }
    var cfg = modules.config && modules.config.ACTIVITY_ENGINE_CONFIG;
    return (cfg && cfg.week7ApiBaseUrl) || '';
  }

  function isWeek7ApiConfigured() {
    var url = getWeek7ApiUrl();
  ...
  function renderSubmitPanel(options) {
    var modules = getModules();
    var hostId = options.hostId || 'w7-submit-host';
    ...
    courseHost.id = 'w7-course-details';
    ...
    summary.id = 'w7-submission-summary';
    ...
    if (!isWeek7ApiConfigured()) {
      ...
      setMessage(
        'w7-submit-status',
        'Your work has been assessed and saved in this browser only. The Week 7 Apps Script collector is not configured yet, so formative results are not recorded remotely.',
        'info'
      );
    }
  ...
  function buildWeek7Payload(activity, learner, options, attemptId, attemptNumber) {
    return {
  ...
  function postWeek7Submission(payload) {
    var url = getWeek7ApiUrl();
    if (!url || !/\/exec\/?$/.test(url)) {
      return Promise.reject(new Error('Week 7 API URL is not configured.'));
    }
    ...
    return fetch(url, {
      ...
      return response.text().then(function (text) {
        ...
        try {
          ...
          throw new Error('The Week 7 API returned an unexpected response.');
        }
  ...
  function submitResult(options) {
    var modules = getModules();
    if (submitting) {
      setMessage('w7-submit-status', 'Submission already in progress. Please wait.', 'warning');
      return;
    }
    ...
    if (!activity) {
      ...
      console.error('[Week7Submit] Missing registry entry for', options.activityId);
      return;
    }
    ...
    modules.learner.showValidationSummary('w7-submit-errors', validation);
    if (!validation.valid) {
      setMessage('w7-submit-status', 'Check your details and try again.', 'warning');
      return;
    }
    ...
    if (!Number.isInteger(score) || score < 0 || score > total) {
      ...
      console.error('[Week7Submit] Invalid score', score, total);
      return;
    }
    ...
    if (total !== activity.maximumScore) {
      ...
      console.error('[Week7Submit] Total mismatch', total, activity.maximumScore);
      return;
    }

    submitting = true;
    var btn = document.getElementById('w7-btn-submit');
    if (btn) {
      btn.disabled = true;
      btn.textContent = 'Submitting…';
    }
    setMessage('w7-submit-status', 'Submitting your formative result…', 'info');

    var payload = buildWeek7Payload(
      activity,
      validation.learner,
      {
        score: score,
      ...
      .then(function (result) {
        ...
        if (envelope.ok === true && (envelope.recorded === true || envelope.duplicate === true)) {
          ...
          if (modules.learner.renderSubmissionSummary) {
            modules.learner.renderSubmissionSummary('w7-submission-summary', {
          ...
          if (typeof options.onSubmitted === 'function') {
            try {
              ...
              console.error('[Week7Submit] onSubmitted callback failed', callbackErr);
            }
          }
          return;
        }
        ...
        setMessage('w7-submit-status', 'Your result was not saved. ' + detail, 'error');
      })
      .catch(function (err) {
        console.error('[Week7Submit] Submission failed', err);
        if (btn) {
          btn.disabled = false;
          btn.textContent = 'Submit formative result';
        }
  ...
  global.Unit3Week7Submit = {
    renderSubmitPanel: renderSubmitPanel,
    submitResult: submitResult,
    isWeek7ApiConfigured: isWeek7ApiConfigured
  };
})(window);
