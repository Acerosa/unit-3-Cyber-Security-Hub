/**
 * Shared Week 5 submission helper.
 * Posts JSON results to the Week 5 Apps Script /exec URL.
 */
...
(function (global) {
  ...
  function getModules() {
    return {
      ...
      progress: global.Unit3Week5Progress,
  ...
  function getWeek5ApiUrl() {
    var modules = getModules();
    if (modules.config && typeof modules.config.getWeek5ApiBaseUrl === 'function') {
      return modules.config.getWeek5ApiBaseUrl();
    }
    var cfg = modules.config && modules.config.ACTIVITY_ENGINE_CONFIG;
    return (cfg && cfg.week5ApiBaseUrl) || '';
  }

  function isWeek5ApiConfigured() {
    var url = getWeek5ApiUrl();
  ...
  function renderSubmitPanel(options) {
    var modules = getModules();
    var hostId = options.hostId || 'w5-submit-host';
    ...
    summary.id = 'w5-submission-summary';
    host.appendChild(summary);

    var activity = resolveActivity(options.activityId);
    modules.learner.renderCourseDetails('w5-course-details', activity);
    modules.learner.renderLearnerForm('w5-learner-form', {
    ...
    if (!isWeek5ApiConfigured()) {
      submitBtn.disabled = true;
      submitBtn.textContent = 'Remote recording not available yet';
      setMessage(
        'w5-submit-status',
        'Your work has been assessed and saved in this browser only. The Week 5 Apps Script collector is not configured yet, so formative results are not recorded remotely.',
        'info'
      );
    }
  ...
  function buildWeek5Payload(activity, learner, options, attemptId, attemptNumber) {
    return {
      ...
      weekNumber: activity.weekNumber || 5,
  ...
  function postWeek5Submission(payload) {
    var url = getWeek5ApiUrl();
    if (!url || !/\/exec\/?$/.test(url)) {
      return Promise.reject(new Error('Week 5 API URL is not configured.'));
    }
    ...
    return fetch(url, {
      ...
      return response.text().then(function (text) {
        ...
        try {
          ...
          throw new Error('The Week 5 API returned an unexpected response.');
        }
  ...
  function submitResult(options) {
    var modules = getModules();
    if (submitting) {
      setMessage('w5-submit-status', 'Submission already in progress. Please wait.', 'warning');
      return;
    }
    ...
    if (!activity) {
      ...
      console.error('[Week5Submit] Missing registry entry for', options.activityId);
      return;
    }
    ...
    modules.learner.showValidationSummary('w5-submit-errors', validation);
    if (!validation.valid) {
      setMessage('w5-submit-status', 'Check your details and try again.', 'warning');
      return;
    }
    ...
    if (!Number.isInteger(score) || score < 0 || score > total) {
      ...
      console.error('[Week5Submit] Invalid score', score, total);
      return;
    }
    ...
    if (total !== activity.maximumScore) {
      ...
      console.error('[Week5Submit] Total mismatch', total, activity.maximumScore);
      return;
    }

    submitting = true;
    var btn = document.getElementById('w5-btn-submit');
    if (btn) {
      btn.disabled = true;
      btn.textContent = 'Submitting…';
    }
    setMessage('w5-submit-status', 'Submitting your formative result…', 'info');

    var payload = buildWeek5Payload(
      activity,
      validation.learner,
      {
        score: score,
      ...
      .then(function (result) {
        ...
        if (envelope.ok === true && (envelope.recorded === true || envelope.duplicate === true)) {
          ...
          if (modules.learner.renderSubmissionSummary) {
            modules.learner.renderSubmissionSummary('w5-submission-summary', {
          ...
          if (typeof options.onSubmitted === 'function') {
            try {
              ...
              console.error('[Week5Submit] onSubmitted callback failed', callbackErr);
            }
          }
          return;
        }
        ...
        setMessage('w5-submit-status', 'Your result was not saved. ' + detail, 'error');
      })
      .catch(function (err) {
        console.error('[Week5Submit] Submission failed', err);
        if (btn) {
          btn.disabled = false;
          btn.textContent = 'Submit formative result';
        }
  ...
  global.Unit3Week5Submit = {
    renderSubmitPanel: renderSubmitPanel,
    submitResult: submitResult,
    isWeek5ApiConfigured: isWeek5ApiConfigured
  };
})(window);
